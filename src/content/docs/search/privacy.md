...
- **Brave**. Some of our search results come from Brave. We share your search term and anonymized location with Brave. We obtain image search results from Brave. If you visit our image results, your browser will load thumbnails from Brave servers. Your access of Brave's image results are governed by Brave's [Privacy Policy](https://search.brave.com/help/privacy-policy), which we review periodically, and offers protections comparable to this document.
- **Ad.net**. We receive syndicated advertisements from Ad.net for some search result pages. These advertisements are received via server-to-server communication. Your browser loads no resources from Ad.net unless you click on an advertisement. We share your search term and your IP address with the last octet zeroed, with Ad.net. If you are a Freespoke Premium user with ad block enabled, Ad.net receives no signal from your searches whatsoever. See [here](https://www.ad.net/privacy-policy/) for Ad.net's privacy policy. Ad.net receives requests only when we expect it likely that an ad will be returned.
- **Gemini**. Our AI results use Google's Gemini models. If you have AI enabled always, or if it is set to Auto mode and our system chooses to provide a generation, your search term and a subset of results are sent to Google Cloud's API to create your answer. We do not provide any information about you to Gemini aside from your search term. This sharing of data is governed by Google's [Paid Use Terms of Service](https://ai.google.dev/gemini-api/terms#data-use-paid), in which Google retains your data for a time in order to detect abuse, but contractually agrees not to use it for any other purposes.
...
One of our processes for understanding your query is called Named-Entity recognition. It's a machine learning task that recognizes people, places, organizations, or companies in your search term.
...
The data described above is retained for 30 days, in order to allow for us to respond to problems in the product and to gain understanding of anonymous usage for improvements.

Beyond 30 days, we store aggregated counts only: We store the total number of searches per hour, along with the number of searches received from logged in or anonymous users, users of our mobile apps and browser extensions, and the number of searches initiated by Freespoke Premium members.

Separately, on a monthly basis, we track the number of times unique search terms appeared, with no additional metadata associated. We do not retain terms which occurred less than five times in a given month.
