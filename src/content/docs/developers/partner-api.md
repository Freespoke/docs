...
If you have been issued a client ID and client secret, exchange them for a short-lived access token using the [OAuth 2.0 Client Credentials Grant](https://datatracker.ietf.org/doc/html/rfc6749#section-4.4). The token endpoint is a standard OpenID Connect token endpoint, so any RFC 6749-compliant OAuth2 library can handle the token exchange and refresh automatically.
